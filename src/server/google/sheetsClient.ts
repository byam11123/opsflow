...
class GoogleIntegrationService {
  ...
  /**
   * Discovers the actual first sheet tab name from a given spreadsheet.
   * Falls back to the provided defaultName if metadata can't be read.
   */
  private async getFirstSheetTabName(spreadsheetId: string, defaultName = 'Form Responses 1'): Promise<string> {
    try {
      const meta = await this.sheetsClient.spreadsheets.get({
        spreadsheetId,
        fields: 'sheets.properties.title',
      });
      const sheets: any[] = meta.data.sheets || [];
      if (sheets.length > 0 && sheets[0].properties?.title) {
        return sheets[0].properties.title;
      }
    } catch (err: any) {
      console.warn(`[Google Sheets] Could not read tab names for ${spreadsheetId}:`, err?.message || err);
    }
    return defaultName;
  }
  ...
  /**
   * Fetches all rows from Vendor Payments first tab in read-only mode
   */
  async fetchFormResponses(): Promise<any[][]> {
    ...
    const spreadsheetId = this.getVendorSpreadsheetId();
    try {
      const tabName = await this.getFirstSheetTabName(spreadsheetId);
      console.log(`[Google Sheets] Vendor tab resolved to: "${tabName}"`);
      ...
      console.warn('[Google Sheets Warning] Could not fetch Vendor responses:', err?.message || err);
      return [];
    }
  }

  /**
   * Fetches all rows from Interbank Transfers first tab in read-only mode
   */
  async fetchInterbankResponses(): Promise<any[][]> {
    ...
    try {
      const tabName = await this.getFirstSheetTabName(spreadsheetId);
      console.log(`[Google Sheets] Interbank tab resolved to: "${tabName}"`);
      const res = await this.sheetsClient.spreadsheets.values.get({
        spreadsheetId,
        range: `${tabName}!A2:K`,
      });
      ...
      console.warn('[Google Sheets Warning] Could not fetch Interbank responses:', err?.message || err);
      return [];
    }
  }

  /**
   * Fetches all rows from Beneficiaries first tab in read-only mode
   */
  async fetchBeneficiaryResponses(): Promise<any[][]> {
    ...
    try {
      const tabName = await this.getFirstSheetTabName(spreadsheetId);
      console.log(`[Google Sheets] Beneficiary tab resolved to: "${tabName}"`);
      const res = await this.sheetsClient.spreadsheets.values.get({
        spreadsheetId,
        range: `${tabName}!A2:O`,
      });
      ...
      console.warn('[Google Sheets Warning] Could not fetch Beneficiary responses:', err?.message || err);
      return [];
    }
  }
